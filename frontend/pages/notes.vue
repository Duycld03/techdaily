...
<template>
  <div class="max-w-4xl mx-auto p-4 sm:p-6 md:p-10 space-y-6 sm:space-y-8 bg-slate-50 dark:bg-canvas transition-colors duration-200">
    <!-- Header -->
    ...
    <div class="space-y-4">
      ...
      <div v-if="notesStore.highlights.length > 0" class="relative">
        ...
        <input
          ...
          type="text"
          class="w-full pl-10 pr-9 py-2.5 text-xs sm:text-sm rounded-2xl bg-white dark:bg-canvas-subtle border border-slate-200/80 dark:border-white/[0.08] text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-brand-500 transition-all shadow-sm"
      ...
      <div v-else-if="filteredHighlights.length > 0" class="space-y-4">
        <div
          ...
          :key="item.id"
          class="glass-card p-5 sm:p-7 rounded-2xl sm:rounded-3xl border-slate-200/80 dark:border-white/[0.08] hover:border-brand-400 dark:hover:border-brand-500/30 transition-all space-y-3.5 sm:space-y-4 shadow-md dark:shadow-none"
        >
          <!-- Reference bar -->
          ...
          <div class="p-4 sm:p-5 rounded-2xl bg-slate-50 dark:bg-canvas-subtle border-l-4 border-brand-500 text-sm md:text-base text-slate-800 dark:text-slate-200 leading-relaxed font-sans italic">
            "{{ item.selectedText }}"
          </div>
          ...
          <div
            ...
            class="glass-panel p-4 rounded-2xl border border-brand-300/80 dark:border-brand-500/30 space-y-3 animate-in fade-in duration-150"
          >
            ...
            <textarea
              ...
              rows="3"
              class="w-full text-xs sm:text-sm bg-white dark:bg-canvas-elevated border border-slate-300 dark:border-white/[0.08] rounded-xl p-3 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-brand-500 transition-all resize-none"
            ...
            <input
              ...
              type="text"
              class="w-full text-xs sm:text-sm bg-white dark:bg-canvas-elevated border border-slate-300 dark:border-white/[0.08] rounded-xl px-3 py-2 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-brand-500 transition-all"
              :placeholder="$t('reader.tags_placeholder')"
              @keydown.enter.prevent="saveEditing(item)"
            />

            <div class="flex items-center justify-end gap-2 pt-1 border-t border-slate-200/80 dark:border-white/[0.08]">
              <button
                @click="cancelEditing"
                class="px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-canvas-elevated transition-colors whitespace-nowrap shrink-0"
              >
                {{ $t('notes.cancel_edit') }}
              </button>
              <button
                ...
                class="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-brand-600 hover:bg-brand-500 text-white text-xs font-semibold shadow-sm transition-all whitespace-nowrap shrink-0 disabled:opacity-50"
              >
          ...
          <template v-else>
            ...
            <div
              v-if="item.note"
              class="p-4 rounded-2xl bg-brand-50/60 dark:bg-brand-950/30 border border-brand-100 dark:border-brand-900/50 space-y-1.5"
            >
              <div class="flex items-center gap-1.5 text-xs font-bold text-brand-700 dark:text-brand-400">
      ...
      <div v-if="filteredHighlights.length > 0" class="pt-4 space-y-4">
        ...
        <div v-if="notesStore.currentPage < notesStore.totalPages" class="flex justify-center">
          <button
            ...
            class="inline-flex items-center gap-2 px-6 py-2.5 rounded-2xl bg-white dark:bg-canvas-subtle border border-slate-200/80 dark:border-white/[0.08] hover:border-brand-500 text-slate-700 dark:text-slate-300 hover:text-brand-400 font-semibold text-xs sm:text-sm shadow-sm transition-all disabled:opacity-50 cursor-pointer"
          >
