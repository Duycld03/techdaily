...
onMounted(async () => {
  ...
  const queryChunk = route.query.chunkOrder ? parseInt(route.query.chunkOrder as string, 10) : undefined
      ...
      <template v-if="focusStore.data?.pacer">
        ...
        <div class="flex items-center gap-2 sm:gap-3 min-w-0 max-w-xl">
          ...
          <div ref="bookMenuRef" class="relative">
            ...
            >
              <BookOpen class="w-3.5 h-3.5 sm:w-4 sm:h-4 text-brand-600 dark:text-brand-400 shrink-0" />
              <span class="max-w-[120px] sm:max-w-[200px] md:max-w-[240px] truncate">
          ...
          <!-- Progress Pill -->
          <div class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700/80 text-xs font-semibold text-slate-700 dark:text-slate-300 max-w-xs truncate shadow-sm whitespace-nowrap shrink-0">
            <span class="truncate max-w-[140px] md:max-w-[180px]">{{ focusStore.data.pacer.chapterTitle }}</span>
            <span class="text-slate-400 dark:text-slate-500">•</span>
            <span class="font-bold text-brand-600 dark:text-brand-400">
              {{ $t('pacer.slice') }} {{ focusStore.data.pacer.currentChunkOrder }}/{{ focusStore.data.pacer.totalChunks }}
            </span>
            <span class="text-slate-400 font-mono">({{ focusStore.data.pacer.progressPercentage }}%)</span>
          </div>
        </div>
    ...
    <div v-else-if="focusStore.data" class="flex-1 flex flex-col md:flex-row overflow-hidden">
      ...
      >
        <InterviewChallengePane
          ...
          :chapter-title="focusStore.data.pacer?.chapterTitle || focusStore.data.documentChunk?.chapterTitle"
        />
      </div>
    </div>
  </div>
