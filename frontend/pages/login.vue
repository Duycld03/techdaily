...
import { BookOpen, Lock, Mail, User, ArrowRight, AlertCircle, Eye, EyeOff, Sparkles, Terminal, Layers, Brain } from 'lucide-vue-next'
...
<template>
  <div class="min-h-[calc(100dvh-3.5rem)] sm:min-h-[calc(100dvh-3.75rem)] flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-slate-50 dark:bg-canvas transition-colors duration-200">
    <div class="w-full max-w-5xl mx-auto grid lg:grid-cols-12 gap-8 lg:gap-12 items-center py-6 px-4">
      ...
      <div class="lg:col-span-5 hidden lg:block space-y-6">
        ...
        <div class="space-y-3">
          ...
          <p class="text-sm lg:text-base text-slate-600 dark:text-slate-400 leading-relaxed">
            {{ locale === 'vi'
              ? 'Nâng cao năng lực kỹ thuật và tư duy kiến trúc thông qua các tình huống thực tế, ôn tập ngắt quãng SM-2 và tài liệu chuẩn hóa.'
              : 'Accelerate your transition to Senior & Staff Engineer with scenario drills, spaced repetition recall, and curated system architecture slices.'
            }}
          </p>
        </div>
        ...
        <div class="space-y-3.5">
          ...
          <div class="bg-white/40 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.06] rounded-xl p-3.5 flex items-start gap-3 transition-colors">
            <div class="w-9 h-9 rounded-lg bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-400 flex items-center justify-center shrink-0 mt-0.5">
              <Layers class="w-4 h-4" :stroke-width="1.75" />
            </div>
            <div class="min-w-0 flex-1">
              <h3 class="text-sm font-bold text-slate-900 dark:text-white leading-snug">
                {{ locale === 'vi' ? 'Tình Huống Kiến Trúc Thực Tế' : 'Scenario Architecture Drills' }}
              </h3>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                {{ locale === 'vi' ? 'Thử thách thiết kế hệ thống và giải quyết bài toán kiến trúc quy mô lớn.' : 'Real-world system design tradeoffs and architecture decision challenges.' }}
              </p>
            </div>
          </div>
          ...
          <div class="bg-white/40 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.06] rounded-xl p-3.5 flex items-start gap-3 transition-colors">
            ...
            <div class="min-w-0 flex-1">
              <h3 class="text-sm font-bold text-slate-900 dark:text-white leading-snug">
                {{ locale === 'vi' ? 'Ghi Nhớ Chủ Động SM-2' : 'Spaced Repetition Mastery' }}
              </h3>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                {{ locale === 'vi' ? 'Thuật toán SM-2 tối ưu hóa khả năng ghi nhớ dài hạn các khái niệm cốt lõi.' : 'SM-2 cognitive algorithm to retain core engineering fundamentals long-term.' }}
              </p>
            </div>
          </div>
          ...
          <div class="bg-white/40 dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.06] rounded-xl p-3.5 flex items-start gap-3 transition-colors">
            ...
            <div class="min-w-0 flex-1">
              <h3 class="text-sm font-bold text-slate-900 dark:text-white leading-snug">
                {{ locale === 'vi' ? 'Lát Cắt Tài Liệu Chuẩn Hóa' : 'Daily System Design Slices' }}
              </h3>
              <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                {{ locale === 'vi' ? 'Trích đoạn tài liệu chuẩn hóa 3-5 phút mỗi ngày, tập trung và súc tích.' : 'Curated 3-5 minute authoritative documentation excerpts tailored for engineers.' }}
              </p>
            </div>
          </div>
        </div>
