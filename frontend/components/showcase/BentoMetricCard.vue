...
const deltaClass = computed(() => {
  ...
  if (props.deltaTrend === 'down') return 'text-red-600 dark:text-red-400'
    ...
    <div class="flex items-start justify-between gap-3">
      <div class="space-y-0.5">
        ...
        <p class="flex items-baseline gap-1">
          ...
          <span v-if="unit" class="text-sm font-medium text-slate-400 dark:text-slate-500">{{ unit }}</span>
        </p>
      </div>
      <span :class="['flex h-9 w-9 shrink-0 items-center justify-center rounded-lg', accentClasses.icon]">
