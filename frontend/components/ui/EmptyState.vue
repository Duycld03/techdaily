...
<template>
  <div
    class="glass-card flex flex-col items-center justify-center gap-3 px-5 py-6 text-center max-h-[220px]"
  >
    <div
      :class="['flex h-11 w-11 items-center justify-center rounded-2xl ring-4', ring]"
    >
      <component :is="icon" class="h-5 w-5" :stroke-width="1.5" />
    </div>
    <div class="space-y-1">
      <h3 class="text-sm font-bold text-slate-900 dark:text-white">{{ heading }}</h3>
      <p class="max-w-xs text-xs text-slate-500 dark:text-slate-400">{{ description }}</p>
    </div>
    <button
      type="button"
      :class="[
        'h-9 rounded-xl px-4 text-xs sm:text-sm font-bold text-white shadow-sm transition-colors cursor-pointer',
        button
      ]"
      @click="emit('action')"
    >
