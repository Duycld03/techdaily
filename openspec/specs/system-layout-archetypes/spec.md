...
2. **Elimination of Side & Bottom Voids**:
   - The layout container SHALL utilize `max-w-7xl` or full-width fluid bounds, eliminating $> 400\text{px}$ horizontal dead margins on 1080p desktop displays.
...
- **THEN** the layout card fills the full available height and width of the content region, its content stays anchored top-left, and no empty vertical void appears below the card and no $> 400\text{px}$ horizontal margin appears beside it.
