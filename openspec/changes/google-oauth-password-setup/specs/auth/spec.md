# Delta Spec: Hybrid Authentication & Password Setup for Google Accounts
...
#### Scenario: Unauthenticated user requests password initialization on login page
- **WHEN** an unauthenticated visitor on `/login` submits their Google email via the "Set Password via Email" flow
- **THEN** the system generates a time-limited cryptographically secure token and sends a password setup instruction email/link.
