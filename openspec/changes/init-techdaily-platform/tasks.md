...
- [x] 3.4 Implement AI Document Chunking Service with pgvector embeddings for URL / Markdown imports.
...
- [x] 4.2 Implement `LibraryEndpoints` (`GET /books`, `POST /import`).
- [x] 4.3 Implement `ReviewEndpoints` (`GET /deck`, `POST /grade`).
- [x] 4.4 Implement `NotesEndpoints` (`GET /highlights`, `POST /highlights`).
...
- [x] 5.4 Build `/library` Book Cards and Import Document modal (preserving source language).
