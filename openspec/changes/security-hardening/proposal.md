...
- Add missing `POSTGRES_PASSWORD` to `.env.example`

### Authentication & Token Security

- **BREAKING**: Reduce JWT access token lifetime from 30 days to 60 minutes
- Introduce refresh token rotation (new `RefreshToken` entity, `/api/v1/auth/refresh` endpoint, `/api/v1/auth/revoke` endpoint) with concurrency-safe atomic rotation and family-based reuse detection
- Refresh token stored in HttpOnly Secure cookie to prevent XSS exfiltration
- Update frontend auth flow to use refresh tokens transparently; every browser request that relies on the refresh cookie must use `credentials: include`, including cross-origin development/API configurations
...
- Configure the refresh-cookie cross-origin contract consistently: `credentials: include` on the frontend and `AllowCredentials()` with explicit origins on the backend; `Secure` cookies require HTTPS in any environment where refresh is exercised
...
- `refresh-tokens`: Refresh token rotation with concurrency-safe atomic rotation, family-based reuse detection, per-family revocation, HttpOnly cookie storage, and automatic frontend token renewal
...
- **Backend**: New `RefreshToken` entity + EF migration, new `CreatedByUserId` column on `DocumentBook` + data migration, new auth endpoints, transactionally atomic refresh rotation (conditional consume plus successor insert), modified PasswordHasher (existing passwords remain verifiable via stored iteration count), modified Program.cs startup, modified endpoint registrations including explicit `403` ownership mapping, modified UrlSecurityValidator
- **Frontend**: Auth store and API client updated for refresh token flow (HttpOnly cookie plus `credentials: include`), access token cookie duration changed, login/register/password-change pages updated for 8-char minimum, 3 pages fixed for XSS
- **Database**: New `RefreshTokens` table, new `CreatedByUserId` nullable FK on `DocumentBooks`, existing user passwords rehashed on next login
- **DevOps**: `.env.example` updated, `docker-compose.prod.yml` secret env vars updated
