...
2. **Dedicated View Mode Switcher & Filter Chips:**
   - The View Mode Switcher (`[ 🌐 Khám Phá ]` / `[ 🔖 Đã Lưu (N) ]`) SHALL render with glass pill styling (`bg-white dark:bg-canvas-subtle border-slate-200/80 dark:border-white/[0.08]`), with active tab styled in Iris Violet (`bg-brand-600/15 text-brand-400 border-brand-500/30 font-bold`).
