...
3. **Stable Top-Aligned 50/50 Bento Generation Studio with Initial Equal Height**:
   ...
   - **Initial Equal Height Baseline**: When "Luyện Đề Theo Sách" is in its default unselected state (`!isGrounded`), the left Bento card SHALL establish a minimum height matching the rendered height of the right Bento card (via `lg:min-h-[415px]` and reactive element size tracking), ensuring both cards share the exact same top and bottom baseline (`ngang nhau`).
   - **Single-Side Downward Expansion**: Toggling the book-grounded mode (`isGrounded`) and expanding the book selector dropdown SHALL expand only the Topic & Context Hub vertically and SHALL NOT force vertical stretching or downward displacement of the primary generation button in the sibling Action Hub.
...
- **THEN** both the Topic & Context card and the Seniority & Action card render with identical top and bottom baselines
